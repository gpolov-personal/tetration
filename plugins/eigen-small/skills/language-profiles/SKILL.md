...
description: "Language-specific toolchain mappings, detection rules, and adaptation notes for the eigen-small pipeline. Load when any command needs to detect project languages, resolve toolchain commands, or adapt structural patterns for non-Python projects."
---

# Language Profiles — Engineering Reference

This is the single source of truth for language-specific toolchain mappings and adaptation notes. eigen-small commands reference this skill when detecting languages, resolving toolchain commands, or adapting structural patterns. (The "swarm" framing below is inherited reference context — eigen-small builds each epic directly, not via swarm workers — but the toolchain tables and adaptation notes apply verbatim.)
...
> **eigen-small note:** Of the skills named below, only `security-best-practices` is **bundled locally** in eigen-small (`skills/security-best-practices/`). The other names are external/optional references — eigen-small does not execute the bundled-path "Lookup Procedure" (it consumes only the toolchain tables + adaptation notes above), so a missing sibling skill is never a hard dependency.
