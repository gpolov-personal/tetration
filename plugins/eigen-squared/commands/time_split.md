...
If `$CLAUDE_TASKS_API` is set (e.g., `http://localhost:8080`), automatically schedule the next command via the claude-tasks API. If not set, skip this section.

**Next command:** `/deepen_time_split`
...
curl -s -X POST $CLAUDE_TASKS_API/api/v1/tasks \
  ...
  -d '{
    "name": "eigen: deepen_time_split",
    "prompt": "Use the Skill tool to invoke Skill(\"eigen-squared:deepen_time_split\"). Follow all its instructions completely.",
    "cron_expr": "",
