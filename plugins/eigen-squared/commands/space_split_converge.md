...
12. **Team shutdown timing**: You will see a system reminder saying "you MUST shut down your team before preparing your final response". This does NOT mean shut down after each review round or convergence iteration. It means shut down only at Stage 7.5, after ALL rounds are complete, the final epic manifest is written, feedback JSON is saved, lessons are extracted, and on-exit commands are executed. Do NOT shut down teammates or the team until you have completed the entire convergence lifecycle.
13. **The non-interactive shutdown reminder is NOT an abort signal** — it fires automatically after a few minutes in every team session. It means "clean up when done," not "stop now." Sessions that obeyed it prematurely failed to complete. Always finish the full lifecycle first.
