---
name: lite_swarm
description: Orchestrate parallel swarm execution of an epic plan across autonomous teammates (lite, single-phase)
---

# Lite Swarm Orchestrator — Staff Engineer / Tech Lead
...
You are the **swarm leader** — a Staff Engineer / Tech Lead responsible for orchestrating the parallel execution of an epic plan by a team of autonomous teammates. You have full context of the plan, all tasks, and all architectural decisions. Your teammates consult you for technical guidance, and you coordinate their work to ensure correctness, consistency, and progress.

**Scope**: one epic at a time (`P1.E<M>`). lite is single-phase — the `P1.` prefix is retained in branch/team names for consistency with the integration branch `feat/P1.E<M>`, but the CLI accepts only `--epic <M>`.

These instructions are **language-aware** — workers create real code in the project's language. Load the `language-profiles` skill (from eigen-squared) for detection, toolchain commands, and adaptation notes (file ownership model, import rules, stub lifecycle, test categorization).
...
```bash
eigen-lite get-context lite_swarm --epic <M> --json
```
...
**If the CLI errors, STOP.** The CLI resolves the integration branch from pipeline state.

| Field | Meaning |
|-------|---------|
| `epic` | Which epic's swarm to orchestrate. |
| `branch` | Integration branch (`feat/P1.E<M>`). You must be on this branch. |
| `manifest_path` | Path to swarm-manifest.json (relative to `$EIGEN_ROOT/eigen_initiative/`). |
| `is_e2e_epic` | `true` iff this is the last, E2E-only epic in the phase. |
...
You will:
1. Verify you are on the integration branch `feat/P1.E<M>` (created by `/lite_plan` Stage E)
...
- **Branch isolation**: ALL teammates operate on the `feat/P1.E<M>` branch. No additional branches.
- **Integration branch required**: This command MUST run while the `feat/P1.E<M>` branch is checked out. Running from `$EIGEN_BRANCH` directly risks modifying the default branch.
...
In lite, the **E2E Testing epic** (produced by `/lite_plan` Stage D when `is_e2e_epic: true`) is the last epic. It:
- Has no features — its scope is writing and running full initiative-level E2E tests
- Is blocked by all feature epics (they must merge first)
- Includes an **Infrastructure Requirements** section describing what the E2E tests need (Docker, emulators, dev servers, etc.)
- Goes through the normal pipeline: `lite_swarm` (this command) followed by `lite_review`

When lite_swarm runs for the E2E Testing epic (`is_e2e_epic: true` in context), it operates like any other epic — workers create tasks (infrastructure setup, E2E test files) and the integration step wires them together — PLUS Stage 4.8 runs a cross-epic fix loop against feature code already merged to `$EIGEN_BRANCH`.

**The E2E Testing epic is the PRIMARY learning opportunity for the initiative.** Real infrastructure, real connections, cross-component failures, and integration patterns discovered here are the most valuable lessons for future initiatives. Workers in this epic should document any integration surprises, infrastructure gotchas, and cross-component patterns thoroughly in their working notes. All findings from this epic's review (P1, P2, and P3) are captured as lessons — unlike feature epics where only P1 findings become lessons.

For feature epics, the workers' validation tests (from the TDD workflow) serve as the primary quality gate. The full initiative-level E2E suite is written and run by the E2E Testing epic's swarm.
...
The CLI context (from On Entry) provides `branch`, `epic`, and `manifest_path`.
...
Print: `Detected Epic <epic> (P1.E<epic>) from branch <branch>.`

### 0.2 Read Parent Epic Context

1. Read the epic file at `eigen_initiative/phases/phase_1/epic_<M>/epic.md`. Extract title, description, features, validation criteria.
2. Read the manifest at the manifest path. Extract `plan_file`. Read the plan file.
3. List task files in `eigen_initiative/phases/phase_1/epic_<M>/tasks/`.
...
1. **Detect languages**: Follow the `language-profiles` skill detection table (loaded cross-plugin from eigen-squared). A project may have multiple languages.
...
- `integration_branch` — `feat/P1.E<M>`
...
```
Create an agent team called "swarm-P1.E<M>" for parallel execution of development tasks.
```
...
```javascript
TaskCreate({
  subject: "[INTEGRATION] Shared files for P1.E<M>",
...
YOUR ASSIGNMENT:
- Task specification: eigen_initiative/phases/phase_1/epic_<M>/tasks/<task_file>
...
5. Verify current branch is `feat/P1.E<M>`.
...
- **fix loop exhausted** (Stage 4.7, 4.8): Accept the current state and proceed to PR creation. Document unresolved failures in a `[DECISION-AUTONOMOUS]` task. `/lite_review` will capture them as findings.
...
All `[DECISION-AUTONOMOUS]` tasks will be visible in the PR summary and to `/lite_review`, which can create fixup tasks if any decision was wrong.
...
- If a provider completed but the file still looks like a stub: create `[DECISION-AUTONOMOUS]` task noting the unreplaced stub, mark provider task as "failed", continue with integration (the stub will cause test failures that `/lite_review` will capture). If `$HUMAN_SWARM_FALLBACK` is `true`: escalate to user instead.
...
3. If tests fail here, it means a consumer's Step C missed something. Create `[BLOCKER]` and `[DECISION-AUTONOMOUS]` task with failure details, proceed to integration anyway (failures will surface in post-integration tests and be captured by `/lite_review`). If `$HUMAN_SWARM_FALLBACK` is `true`: escalate to user instead.
...
"You are the integration specialist for epic P1.E<M>.

BRANCH VERIFICATION (MANDATORY FIRST ACTION):
VERIFY you are on the correct branch:
  1. Run: git rev-parse --abbrev-ref HEAD — must be feat/P1.E<M>
...
7. Commit: git add <shared_files> && git commit -m 'feat(P1.E<M>): integrate shared files'
...
6. If failures remain AND iteration >= 3:
   Accept current state. Create `[DECISION-AUTONOMOUS]` task: "Post-integration fix loop exhausted after 3 iterations. <N> failures remain: <list>. Proceeding to PR creation. Failures will be captured by /lite_review." Proceed to Stage 5. If `$HUMAN_SWARM_FALLBACK` is `true`: escalate to user with options (accept / guidance / manual control) instead.

### 4.8 E2E Validation Fix Loop (E2E Testing Epic ONLY)

**Condition**: Only activate when `is_e2e_epic == true` in the On-Entry context (the last, E2E-only epic in the initiative).

**Skip for feature epics** — feature epics proceed directly to Stage 5 after integration.

This phase runs the full E2E test suite assembled by the E2E Testing epic's workers and implements a cross-epic fix loop when CODE_BUGs are found in feature code from already-merged epics.
...
```javascript
TaskCreate({
  subject: "[E2E-VALIDATION] E2E fix loop P1.E<M>",
...
In lite, "cross-epic" means across the feature epics within this single phase. E2E failures can originate in ANY feature epic's code.

1. Read the pipeline state (`eigen-lite status --json`) to identify ALL feature epics
...
2. Spawn a fix worker on the integration branch (where all feature code is already merged):
   ...
   An E2E test is failing. This failure was attributed to your code from epic P1.E<M>.
   - Failing test: <test_name>
   - Error: <error_type>: <error_message>
   - Stack trace files: <files>
...
5. **Iteration-specific behavior**:
   ...
   - **Iteration 3 (Final)**: No new fix workers spawned. If failures remain:
     Accept current state. Create `[DECISION-AUTONOMOUS]` task: "E2E fix loop exhausted after <max_iterations> iterations. <N> failures remain: <list>. Proceeding to PR creation. Failures will be captured by /lite_review." Proceed to Stage 5. If `$HUMAN_SWARM_FALLBACK` is `true`: escalate to user with options instead.
...
gh pr create --base $EIGEN_BRANCH --head feat/P1.E<M> \
  --title "feat: P1.E<M> — <epic_name>" \
  --body "## Epic P1.E<M>: <epic_name>
...
```bash
eigen-lite complete lite_swarm --epic <M> --pr-url <pr_url> --pr-number <pr_number>
eigen-lite commit-state --message "chore: record PR for P1.E<M> in pipeline state"
```

This is the handoff point — `/lite_review` reads `lite_swarm` state from the integration branch to detect the PR and track review iterations. Both commands run on the same branch.

### 5.5 Cleanup Team

```
Clean up the team "swarm-P1.E<M>".
```

### 5.6 Report to User

```
=== Lite Swarm Execution Report — P1.E<M> ===

Epic: P1.E<M> — <epic_name>
Branch: feat/P1.E<M>
PR: <pr_url>
...
Next steps:
  ...
  2. Run /lite_review to get an automated review
  3. If this was the last feature epic, run /lite_swarm again
     — it will auto-detect the E2E Testing epic (the last epic with is_e2e_epic: true)
     which writes and runs the full initiative-level E2E test suite
```
...
- **Branch isolation**: all teammates operate on the `feat/P1.E<M>` branch, no additional branches
- **Must run from integration branch**: this command verifies the branch on entry — if not on `feat/P1.E<M>`, it STOPs with instructions
- **Testing philosophy**: real dependencies, minimal mocks, always — enforce this with workers
- **E2E Testing epic**: when running the E2E Testing epic (`is_e2e_epic: true`), workers create infrastructure and E2E tests as their normal tasks. After integration, the leader runs the E2E fix loop (Stage 4.8) to attribute and fix cross-epic failures before creating the PR.
...
- [ ] Team cleanup called
- [ ] PR created from `feat/P1.E<M>` → `$EIGEN_BRANCH`
- [ ] On Exit CLI commands executed (`eigen-lite complete` + `eigen-lite commit-state`)
